...
ecewo provides JavaScript-like `ecewo_set_timeout()` and `ecewo_set_interval()` functions, implemented with the libuv event loop, for non-blocking time-based operations.
...
The `ecewo_set_timeout()` function executes a callback function once after a specified delay.
...
void on_user_register(const char *username) {
  ...
  ecewo_set_timeout(show_welcome_notification, 2000, name_copy);
...
session_timers_t* setup_session_timeout(int user_id) {
  ...
  *uid = user_id;
  timers->warning_timer = ecewo_set_timeout(warn_session_expiring, 25 * 60 * 1000, uid);
  
  // Expire after 30 minutes
  timers->expiry_timer = ecewo_set_timeout(expire_user_session, 30 * 60 * 1000, timers);
...
The `ecewo_set_interval()` function executes a callback function repeatedly at specified intervals.
...
**Important:** For `ecewo_set_interval`, you **MUST call `ecewo_clear_timer()`** to stop the timer. Otherwise, it runs forever (until server shutdown).
...
Timer *start_time_display(void) {
  return ecewo_set_interval(print_current_time, 10000, NULL);
}
...
auto_save_context_t* start_auto_save(int user_id, const char *doc_id) {
  ...
  ctx->auto_save_timer = ecewo_set_interval(auto_save_document, 30000, ctx);
...
- `timer`: Timer handle to cancel (from `ecewo_set_timeout` or `ecewo_set_interval`)

**When to Use:**

- **Always** for `ecewo_set_interval` timers (to stop repetition)
- For `ecewo_set_timeout` timers **only if cancelling before they fire**
- **Never** for `ecewo_set_timeout` timers after they've already fired (auto-cleaned)
...
void start_hello_timer(void) {
  hello_timer = ecewo_set_interval(say_hello, 2000, NULL);
...
email_schedule_t* schedule_email(const char *to, const char *subject, 
  ...
  schedule->timer = ecewo_set_timeout(send_email, delay_ms, schedule);
...
> [!NOTE]
>
> For `ecewo_set_timeout`, you **do NOT need to call `ecewo_clear_timer()`**. The timer automatically cleans up after executing. Only call `ecewo_clear_timer()` if you need to cancel before it fires.
>
...
ecewo_set_timeout(callback, 1000, data);
// callback will free(data)

// Cancelled execution
char *data = strdup("hello");
Timer *timer = ecewo_set_timeout(callback, 1000, data);
ecewo_clear_timer(timer);
...
int main(void) {
  ...
  ecewo_set_interval(check_service_health, 30000, &db_check);
...
void handle_request(int user_id) {
  ...
  if (limit->request_count == 1) {
    ...
    *limit_copy = *limit;
    ecewo_set_timeout(reset_rate_limit, 60000, limit_copy);
  }
}
```
