...
Controls connection handling and timeouts. These are **runtime fields** on the `Ecewo` struct. Set them after `ecewo()` and before `server_listen()`.
...
int main(void) {
  ...
  // Runtime configuration (set before server_listen())
  app->max_connections    = 5000;
  app->idle_timeout_ms    = 45000;
  app->request_timeout_ms = 20000;
