...
void admin_only(Req *req, Res *res, Next next) {
  const char *role = ecewo_get_header(req, "X-User-Role");
...
void custom_timeout_middleware(Req *req, Res *res, Next next) {
  ...
  if (strncmp(path, "/api/", 5) == 0) {
    ecewo_request_timeout(res, 10000);  // 10s
  } else if (strncmp(path, "/upload", 7) == 0) {
    ecewo_request_timeout(res, 300000); // 5min
  }
  // else: no timeout
...
void timeout_middleware(Req *req, Res *res, Next next) {
  ecewo_request_timeout(res, 15000);
  next(req, res);
}
...
There is a specific way to pass the data along the middleware chain: `ecewo_get_context()` and `ecewo_set_context()` functions. We can pass the data to the next middleware or to the handler using them.
...
void protected_handler(Req *req, Res *res) {
  ...
  if (!ctx) {
    ecewo_send_text(res, UNAUTHORIZED, "Unauthorized");
    return;
  }
...
- `ecewo_set_context()` stores the pointer directly (no copy)
