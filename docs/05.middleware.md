...
int main(void) {
  ...
  get(app, "/admin/users", admin_only, users_handler);
  post(app, "/admin/settings", admin_only, validate_settings, settings_handler);
...
int main(void) {
  ...
  get(app, "/path", custom_timeout_middleware, handler);
...
int main(void) {
  ...
  // Apply to all routes
  use(app, logging_middleware);
  use(app, auth_middleware);

  get(app, "/protected", protected_handler);
  // ... rest of setup
}
```
...
int main(void) {
  ...
  use(app, timeout_middleware);

  get(app, "/path1", handler1);
  get(app, "/path2", handler2);
  get(app, "/path3", handler3);
...
`use()` also accepts an optional path prefix as its second argument. The middleware will only run for requests whose path starts with that prefix — similar to Express.js's `app.use('/prefix', fn)`.
...
int main(void) {
  ...
  use(app, "/api", api_only_middleware);

  get(app, "/api/users", users_handler);   // middleware runs
  get(app, "/api/posts", posts_handler);   // middleware runs
  get(app, "/public", public_handler);     // middleware does NOT run
...
- `use(app, "/api", fn)` matches `/api`, `/api/users`, `/api/v2/items`, ...
- `use(app, "/api", fn)` does **not** match `/apiv2` or `/api-old`
- `use(app, "/", fn)` matches every path (equivalent to `use(app, fn)`)
- `use(app, fn)` (no path) is global — runs for all requests
