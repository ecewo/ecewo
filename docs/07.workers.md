...
void compute_handler(Req *req, Res *res) {
  const char *iter_str = ecewo_get_query(req, "iterations");
  ...
  ecewo_send_text(res, ACCEPTED, msg);
}
...
void parallel_handler(Req *req, Res *res) {
  ...
  // Computation will work on background
  ecewo_send_text(res, ACCEPTED, "Calculation started in background");
}
```
...
> [!CAUTION]
>
> If you want to set a route-specific timeout using `ecewo_request_timeout()` in an async handler, you must call it before calling `ecewo_spawn()` or `ecewo_spawn_http()`. DO NOT call `ecewo_request_timeout()` in a worker!
